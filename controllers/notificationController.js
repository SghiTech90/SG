...
const getUpcomingDueDates = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: 'Error fetching upcoming due dates', error: error.message });
    }
};
...
const getPendingTenders = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: 'Error fetching pending tenders', error: error.message });
    }
};
